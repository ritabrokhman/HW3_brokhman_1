...
def calc_global_joint_prob(model, variableValues):
  ...
  for var in model.vars:
    pr *= read_cpt(model, var, variableValues) if variableValues[var] else (1 - read_cpt(model, var, variableValues))
  return pr
...
def calc_query_exact_tree(model, queryVar, queryVal, evidence):
  ...
  norm_factor = prQ_T + prQ_F
  pr_T = prQ_T / norm_factor
  pr_F = prQ_F / norm_factor

  return pr_T if queryVal else pr_F
  # YOUR CODE HERE
  #
